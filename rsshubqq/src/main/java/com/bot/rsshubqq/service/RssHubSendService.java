...
public class RssHubSendService implements Runnable{
    ...
    RssHubSendService(String sendName, RssItem sendItem, RssFeedItem rssFeedItem,TranslateConfig translateConfig,RsshubConfig rsshubConfig,String tempPath){
            ...
            restTemplate.execute(mediaUrl, HttpMethod.GET, null, response -> {
                if (response.getStatusCode() == HttpStatus.OK) {//判断是否响应正常
                    ...
                    if (imagesSuffix != null) {//请求响应为图片并获取后缀名
                        ...
                        imageFile.set(new File(tempPath + fileName + "." + imagesSuffix));
